...
uniform vec3 gLightPos[1];

void main()
{
    vec3 e1 = WorldPos0[2] - WorldPos0[0];
    vec3 e2 = WorldPos0[4] - WorldPos0[0];
    vec3 e3 = WorldPos0[3] - WorldPos0[0];
    vec3 e4 = WorldPos0[5] - WorldPos0[2];
    vec3 e5 = WorldPos0[4] - WorldPos0[2];
    vec3 e6 = WorldPos0[1] - WorldPos0[0];
//    vec3 e6 = WorldPos0[4] - WorldPos0[2];

    vec3 Normal = cross(e1,e2);
    for(int i = 0; i < 1; i++)
    {
        vec3 LightDir = gLightPos[i] - WorldPos0[0];
        ...
        if (dot(Normal, LightDir) > 0.00001) {
            ...
            Normal = cross(e4,e5);
            LightDir = gLightPos[i] - WorldPos0[2];
            ...
            Normal = cross(e2,e6);
            LightDir = gLightPos[i] - WorldPos0[4];
